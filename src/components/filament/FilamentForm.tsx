...
export function FilamentForm({ initial, onClose }: FilamentFormProps) {
  ...
  const [weightKg, setWeightKg] = useState<string>(
    initial ? String(initial.weightKg) : "1",
  );
  const [priceUsd, setPriceUsd] = useState<string>(
    initial ? String(initial.priceUsd) : "",
  );
  ...
  const selectedMaterial =
    state.materials.find((m) => m.id === materialId) ?? null;
  ...
  if (pendingBrandName) {
    const found = state.brands.find(
      (b) => b.name.toLowerCase() === pendingBrandName.toLowerCase(),
    );
    if (found) {
      setBrandId(found.id);
      setPendingBrandName("");
  ...
  if (pendingMaterialName) {
    const found = state.materials.find(
      (m) => m.name.toLowerCase() === pendingMaterialName.toLowerCase(),
    );
    if (found) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          />
          {errors.weightKg && (
            <p className="text-xs text-red-600 mt-0.5">{errors.weightKg}</p>
          )}
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">
          ...
          />
          {errors.priceUsd && (
            <p className="text-xs text-red-600 mt-0.5">{errors.priceUsd}</p>
          )}
        </div>
      </div>
      ...
      <div className="flex justify-end gap-3 pt-2 border-t border-gray-100">
        ...
        <Button type="submit">
          {initial ? "Save Changes" : "Add Filament"}
        </Button>
      </div>
    </form>
  );
